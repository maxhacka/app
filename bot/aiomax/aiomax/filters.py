...
class state(_filter):
    def __init__(self, state: "Any | list[Any]"):
        """
        :param state: State to check (can be a single state or list of states)

        Checks if the user is in the given state(s)
        """
    ...
    def __call__(self, obj: Any):
        ...
        current_state = storage.get_state(user_id)

        # Check if current state matches any of the expected states
        for expected_state in self.states:
            if current_state == expected_state:
                return True
            # Also check string representation for compatibility
            if str(current_state) == str(expected_state):
                return True
