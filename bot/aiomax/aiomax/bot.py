...
class Bot(Router):
    ...
    async def get(self, *args, **kwargs):
        ...
        if self.session is None:
            raise exceptions.AiomaxException("Session is not initialized")
        ...
        try:
            ...
            raise exceptions.NetworkError("Request timeout") from e
        except aiohttp.ClientError as e:
            raise exceptions.NetworkError(str(e)) from e
    ...
    async def send_message(
        ...
        try:
            ...
            event_logger.info(f"📤 {target_id} - message_sent")
    ...
    async def edit_message(
        ...
        try:
            ...
            event_logger.info(f"✏️ {message_id} - message_edited")
    ...
    async def handle_update(self, update: dict):
        ...
        if update_type == "message_created":
            ...
            event_logger.info(f"📨 {message.sender.user_id} - message")
            ...
            for prefix in prefixes:
                ...
                event_logger.info(f' {message.sender.user_id} - command /{name}')
        ...
        if update_type == "message_edited":
            ...
            # Логируем редактирование сообщения
            event_logger.info(f"✏️ {message.sender.user_id} - message_edit")
        ...
        if update_type == "message_removed":
            ...
            event_logger.info(f"🗑️ {payload.user_id} - message_delete")
        ...
        if update_type == "bot_started":
            ...
            event_logger.info(f" {payload.user.user_id} - bot_start")
        ...
        if update_type == "chat_title_changed":
            ...
            # Логируем изменение названия чата
            event_logger.info(f" {payload.user.user_id} - chat_title_change")
        ...
        if update_type == "bot_added" or update_type == "bot_removed":
            ...
            # Логируем добавление/удаление бота
            event_logger.info(f"🤖 {payload.user.user_id} - bot_{update_type.replace('bot_', '')}")
        ...
        if update_type == "user_added" or update_type == "user_removed":
            ...
            # Логируем добавление/удаление пользователя
            event_logger.info(f"👤 {payload.user.user_id} - user_{update_type.replace('user_', '')}")
        ...
        if update_type == "message_callback":
            ...
            event_logger.info(f"🔘 {callback.user.user_id} - button '{callback.payload}'")
            ...
            if handled:
                event_logger.debug(f' {callback.user.user_id} - button_handled')
            else:
                event_logger.debug(f' {callback.user.user_id} - button_unhandled')

        if update_type == "message_chat_created":
            payload = ChatCreatePayload.from_json(update)

            # Логируем создание чата
            event_logger.info(f"💬 {payload.message_id} - chat_created")
    ...
    async def start_polling(
        ...
        async with session:
            ...
            bot_logger.info(
                f" Bot started polling: @{self.username} ({self.id}) - {self.name}"
            )

            # ready event
            event_logger.info(" bot - on_ready_handlers")
            for i in self.handlers["on_ready"]:
                asyncio.create_task(i())

            event_logger.info(" bot - ready")
            ...
            while self.polling:
                try:
                    ...
                    if updates and "updates" in updates:
                        event_logger.debug(f" bot - received_{len(updates['updates'])}_updates")

                        for update in updates["updates"]:
                            await self.handle_update(update)
                    else:
                        event_logger.debug(" bot - no_updates")

                except Exception as e:
                    event_logger.error(f" bot - polling_error: {e}")
                    await asyncio.sleep(3)

                except asyncio.exceptions.CancelledError:
                    event_logger.info(" bot - polling_cancelled")
