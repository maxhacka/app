...
class KeyboardBuilder:
    ...
    def button(
        ...
        """
        Add a button to the keyboard (simplified method like in aiogram)

        :param text: Button text
        :param callback_data: Callback data (for callback buttons)
        :param url: URL (for link buttons)
        """
        if callback_data:
            return self.add(CallbackButton(text, callback_data))
        elif url:
            return self.add(LinkButton(text, url))
        else:
            raise ValueError("Either callback_data or url must be provided")

    def adjust(self, *sizes: int):
        """
        Adjust button layout. Redistributes buttons into rows with specified sizes.

        Example:
            kb.button("1").button("2").button("3").button("4").adjust(2, 1)
            # Results in: [[1, 2], [3], [4]]

        :param sizes: Row sizes (number of buttons per row)
        """
        # Flatten all buttons
        all_buttons = []
        ...
        for size in sizes:
            if button_index >= len(all_buttons):
                break
            row = all_buttons[button_index : button_index + size]
...
# Convenient functions for quick keyboard creation


def inline_keyboard(*rows: list[Button]) -> list[list[Button]]:
    """
    Create inline keyboard from rows of buttons

    Example:
        keyboard = inline_keyboard(
            [CallbackButton("Button 1", "data1"), CallbackButton("Button 2", "data2")],
            [LinkButton("Link", "https://example.com")]
        )
    """
    return list(rows)


def quick_keyboard(*button_specs: tuple[str, str]) -> KeyboardBuilder:
    """
    Quick keyboard builder from button specifications
    ...
    Example:
        ...
        ).adjust(2, 1)  # 2 buttons in first row, 1 in second
    """
    kb = KeyboardBuilder()
    for text, data in button_specs:
        if data.startswith("http://") or data.startswith("https://"):
            kb.add(LinkButton(text, data))
        else:
            kb.add(CallbackButton(text, data))
    return kb
