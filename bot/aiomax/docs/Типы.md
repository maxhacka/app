...
- `vcf_phone: str | None` - ✨ **ОБНОВЛЕНО:** номер телефона контакта. **Автоматически извлекается из `vcf_info` при получении**
...
При получении контакта от пользователя номер телефона автоматически извлекается из VCF формата:
...
async def contact_handler(message):
    for attachment in message.body.attachments:
        if attachment.type == "contact":
            # Номер телефона автоматически извлечён из vcf_info
            phone = attachment.vcf_phone  # Например: "79526465570"
            user_info = attachment.max_info
            
            if phone:
                await message.reply(f"Получен номер: +{phone}")
            
            if user_info:
                await message.reply(f"Контакт: {user_info.name}")
```

**Формат входных данных от API:**
```json
{
  "attachments": [{
    "payload": {
      "vcf_info": "BEGIN:VCARD\r\nVERSION:3.0\r\nTEL;TYPE=cell:79526465570\r\nFN:Имя\r\nEND:VCARD\r\n",
      "max_info": {...}
    },
    "type": "contact"
  }]
...
Библиотека автоматически парсит строку `vcf_info` и извлекает номер телефона из поля `TEL;TYPE=cell:`.
